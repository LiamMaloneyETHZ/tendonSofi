...
passiveness = np.array([0.5,0.5,0.5])
...
control_mode = 2
motorPos = np.zeros((NUM_BODY_SERVOS,1))
motorLoad = np.zeros((NUM_BODY_SERVOS,1))
#motorLoadThresh = np.array([300,500,700])
#passiveCount = np.array([0,0,0])
...
uwAmp = -18171
uwPhase = 0.5903
POS_0_DEG = np.array([1919,3834,4000,884,2900,4072])
...
A = gait_amps
omega_s = gait_spFreq
N = int(NUM_BODY_SERVOS/2)   #number of joints
omega_t = gait_tmFreq
...
for k in range(N):
    ...
    print(angle[k])
    #print(motor_pos_decimal_original)
    #print(motor_pos_decimal)
...
for k in range(N):
    ...
    motor_pos_decimal[2*k+1] = motor_pos_decimal_original[2*k+1]
    if angle[k] >= -gamma[k]:
        motor_pos_decimal[2*k+1] = uwAmp*np.cos(-(-gamma[k])/180*np.pi/2 + uwPhase) + POS_OFFSET[2*k+1] - 100*abs(angle[k]-(-gamma[k]))
    if angle[k] <= gamma[k]:
        motor_pos_decimal[2*k] = uwAmp*np.cos((gamma[k])/180*np.pi/2 + uwPhase) + POS_OFFSET[2*k] - 100*abs(angle[k]-(gamma[k]))
