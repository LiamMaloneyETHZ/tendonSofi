/*
  ...
  DATA sent from RPI to TRANSMITTER that is then sent to RECIEVER. . . Flash TRANSMITTER with this 
...
void loop() {
  ...
  String data_val=SerialUSB.readStringUntil('\n');
  SerialUSB.print("This is the data received via USB from the RPI: ");
  SerialUSB.println(data_val);

  char buffer[64];
  data_val.toCharArray(buffer,sizeof(buffer));

  nrf24.send((uint8_t*)buffer,strlen(buffer));
