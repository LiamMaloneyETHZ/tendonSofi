...
# Import a generated Gait Matrix
mat_data = loadmat('gait_head_straight_40_0.8.mat')  # replace with your actual filename
angleMatRev = np.array(mat_data['angle_traj'])  # replace 'variable_name' with the actual key
angleMat = angleMatRev[::-1,:]  # swap head to be last row
...
motorPosMat = preCalculateAngles(angleMat,*bodyServos)
...
try:
    ...
    movePredetermined(bodyGroupWrite, motorPosMat, *bodyServos, t=0)
    # ============== Initialize Gait =====================
    if G != 0:
        input("Press enter to initialize Gait...")
        movePredetermined(bodyGroupWrite, motorPosMat, *bodyServos)
    ...
    print(range(all_steps))
    for index in range(all_steps):
        start_time = time.time()
        t += dt
        movePredetermined(bodyGroupWrite, motorPosMat, *bodyServos, t=index)
