...
for k in range(int(len(bodyServos)/2)):
    POS_MIN[2*k+1]= uwAmp*np.cos(-max_angle/180*np.pi/2 + uwPhase) + POS_OFFSET[2*k+1]-extra_to_give
    POS_MAX[2*k] = uwAmp*np.cos(max_angle/180*np.pi/2 + uwPhase) + POS_OFFSET[2*k]-extra_to_give

    POS_MIN[2*k] = uwAmp*np.cos(-max_angle/180*np.pi/2 + uwPhase) + POS_OFFSET[2*k]-extra_to_give
    POS_MAX[2*k+1] = uwAmp*np.cos(max_angle/180*np.pi/2 + uwPhase) + POS_OFFSET[2*k+1]-extra_to_give
...
# inputPercent = int(input("Enter percentage constant for tail syringe"))
# percentMove([tls4],extensionPercent=inputPercent)
# groupMove(syrGroupWrite,*syringeTuple)
...
try:
    ...
    for index in range(all_steps):
        ...
        if (presentTime-syringeTimerStart) > 3:
            syrCounter +=1
        ...
        if torqueFlag:
            if (presentTime-syrDisableClock) > 2.9:
