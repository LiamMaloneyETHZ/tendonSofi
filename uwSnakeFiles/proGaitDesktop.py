...
omega_s = 0.2
des_tmpFreq = 0.2
turnOffset = 0 #bound A+turnOffset to 50 degrees
...
softTorqueLimit = True
highTorqueCounterLimit = 1 #instances of high torque occuring every command freq
torqueThreshold = 90 #Percentage of max torque: 1.8 Nm for 2XC430 @ 12V, 1.5 Nm for 2XL430 @ 12V
...
params = {
    ...
    'turnOffset':        turnOffset                 # degrees, dont pass max allowables
}
...
try:
    ...
    for index in range(all_steps):
        ...
        posReadSuccess = readGroupPos(bodyGroupReadPos, *bodyServos)
        loadReadSuccess = readGroupLoad(bodyGroupReadLoad, *bodyServos)
        ...
        if softTorqueLimit:
            for servo in flipped_bodyServos:
                if abs(servo.presLoad/10) > torqueThreshold:
                    servo.highTorqueCounter += 1
                    print(f"Servo {servo.dxlID} Torque is {servo.presLoad/10}%")
                elif not loadReadSuccess:
                    pass 
                else:
                    servo.highTorqueCounter = 0
                if servo.highTorqueCounter >= highTorqueCounterLimit: #0.5 seconds for a command freq of 0.05s
                    overTorque = True
                    servo.torqueDisable()
        if overTorque:
            print("Torque threshold exceeded! Ending gait.")
            stop_time = time.time()
            ...
            break  #This breaks out of the main `for index in range(all_steps)` loop
...
time.sleep(1) #trying to avoid the port being in use?
print("Press any key to disable torque or f to quit")
key = readKey()
if key == 'f':
    pass
else:
...
#torque stays on...
time.sleep(3)
#torqueDisable(*bodyServos) #keep this for batteries   
bodyServos[0].checkVoltage()
